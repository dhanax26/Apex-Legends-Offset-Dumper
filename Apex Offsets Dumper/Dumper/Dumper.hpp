...
namespace Dumper
{
	...
	__forceinline void DumpOffsets()
	{
		...
		auto LocalPlayer = Memory::FindPattern(xorstr_("48 8B 05 ? ? ? ? 48 0F 44 C7"));//PlayerRunCommand_Prediction
		...
		auto GlobalVars = Memory::FindPattern(xorstr_("4C 8D 05 ? ? ? ? 48 8D 15 ? ? ? ? 41 FF D1"));//Client.dll Init() in library client failed.
		...
		auto ViewRender = Memory::FindPattern(xorstr_("48 8B 0D ? ? ? ? 48 8B 01 FF 50 40 48 8B 0D ? ? ? ?"));//_rt_FullFrameFB
		...
		auto ClientState = Memory::FindPattern(xorstr_("48 8D 0D ? ? ? ? E8 ? ? ? ? FF 15 ? ? ? ? 89 05 ? ? ? ?"));//Out of thread local storage!
		...
		auto LevelName = Memory::FindPattern(xorstr_("48 8D 05 ? ? ? ? C3 CC CC CC CC CC CC CC 83 3D ? ? ? ? ?"));//dedicated - mp_lobby
