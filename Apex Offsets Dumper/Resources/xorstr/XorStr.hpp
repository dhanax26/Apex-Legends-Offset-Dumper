...
namespace jm {
    ...
    namespace detail {
        ...
        XORSTR_FORCEINLINE constexpr std::uint32_t key4()
        {
        ...
        XORSTR_FORCEINLINE constexpr std::uint64_t
            load_xored_str8(std::uint64_t key, std::size_t idx)
        {
        ...
        XORSTR_FORCEINLINE std::uint64_t load_from_reg(std::uint64_t value)
        {
            return value;
        }
    ...
    public:
        ...
        XORSTR_FORCEINLINE pointer get()
        {
            return reinterpret_cast<pointer>(_storage);
        }

        XORSTR_FORCEINLINE pointer crypt_get()
        {
            crypt();
